...
namespace ns2
{
    ...
    public:
        ...
        virtual void notify(const Fighter *talker, const string &tmpContent) const = 0; // 通知的一些细节信息
    };
    ...
    public:
        void NotifyWords(const Fighter *talker, const string &tmpContent) const // 其他玩家收到了当前玩家的聊天信息
        {
            if (talker != this)
                talker->ReceiveMessage(this, tmpContent); // 同一个家族的其他玩家也应该收到聊天信息
        }
    ...
    public:
        ...
        void notify(const Fighter *talker, const string &tmpContent) const override // talker是讲话的玩家
        {
...
int main()
{
    ...
    using namespace ns2;
    // 创建游戏玩家
    shared_ptr<Fighter> pplayerobj1 = make_shared<F_Warrior>(10, "1");
    ...
    // 创建通知器
    shared_ptr<Notifier> ptalknotify = make_shared<TalkNotifier>();
